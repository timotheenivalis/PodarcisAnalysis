...
setwd("C:/Users/Timothée/Documents/Studies/PodarcisCBGP/New simulations/BigBD")
...
write.table(results,file="BigBDresults.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(resultsnofix,file="BigBDresultsnofix.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(DistriMtMaxDF,file="BigBDDistriMtMax.txt", sep="\t",quote=F,append=F,col.names=T,row.names=F)


for (nbsimul in 1:6)
{
  Simul<-paste("BigBD",nbsimul,sep="")
  outR<-mtfunc(Simul)
  results<-outR$results
  resultsnofix<-outR$resultsNoFix
  results$Param<-Param[nbsimul+1]
  resultsnofix$Param<-Param[nbsimul+1]
  write.table(results,file="BigBDresults.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(resultsnofix,file="BigBDresultsnofix.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(cbind(rep(Simul,nrow(outR$DistriMtMax)),
                    outR$DistriMtMax,rep(Param[nbsimul+1],nrow(outR$DistriMtMax))),
              file="BigBDDistriMtMax.txt", sep="\t",quote=F,append=T,col.names=F,row.names=F)
}

##############################################################################################
#############################################################################################
par(mfrow=c(1,1))
setwd("C:/Users/Thimothee Admin/Documents/external research/PodarcisCBGP/New simulations/BigBD/")
simul<-read.table(file="BigBDresults.txt",header=T)
Param<-matrix(data=c(0:6),nrow=7,ncol=1)
simul$Param<-Param
