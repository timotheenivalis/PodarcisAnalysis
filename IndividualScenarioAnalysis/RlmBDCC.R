...
setwd("C:/Users/Timothée/Documents/Studies/PodarcisCBGP/Renew simulations/lmBDCC/")
...
write.table(results,file="lmBDCCresults.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(resultsnofix,file="lmBDCCresultsnofix.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(DistriMtMaxDF,file="lmBDCCDistriMtMax.txt", sep="\t",quote=F,append=F,col.names=T,row.names=F)


for (nbsimul in 1:8)
{
  Simul<-paste("lmBDCC",nbsimul,sep="")
  outR<-mtfunc(Simul)
  results<-outR$results
  resultsnofix<-outR$resultsNoFix
  DistriMtMaxDF<-data.frame(rep(Simul,nrow(outR$DistriMtMax)),outR$DistriMtMax)
  names(DistriMtMaxDF)<-c("Simul","DistriMtMax","DistriNuMtMax")
  write.table(results,file="lmBDCCresults.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(resultsnofix,file="lmBDCCresultsnofix.txt", sep="\t",quote=F,append=T,col.names=F)  
  write.table(DistriMtMaxDF,file="lmBDCCDistriMtMax.txt", sep="\t",quote=F,append=T,col.names=F,row.names=F)
}

simul<-read.table(file="lmBDCCresults.txt",header=T)
simulnofix<-read.table(file="lmBDCCresultsnofix.txt",header=T)
