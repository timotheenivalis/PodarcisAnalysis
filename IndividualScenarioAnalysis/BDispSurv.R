...
simOrd<-c("BD1S2","BD1S3","BD1S5","BD3S2","BD3S3","BD3S5","BD6S2","BD6S3","BD6S5")

source(file="C:/Users/Timothée/Documents/Studies/PodarcisCBGP/New simulations/FunctionsSimul.R")
setwd("C:/Users/Timothée/Documents/Studies/PodarcisCBGP/New simulations/BDispSurv/")
...
write.table(results,file="BDSresults.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(resultsnofix,file="BDSresultsnofix.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(DistriMtMaxDF,file="BDSDistriMtMax.txt", sep="\t",quote=F,append=F,col.names=T,row.names=F)
...
for (nbsimul in 2:9)
{
  ...
  write.table(results,file="BDSresults.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(resultsnofix,file="BDSresultsnofix.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(DistriMtMaxDF,file="BDSDistriMtMax.txt", sep="\t",quote=F,append=T,col.names=F,row.names=F)
}


##############################################################################################
#############################################################################################
simul<-read.table(file="BDSresults.txt",header=T)
