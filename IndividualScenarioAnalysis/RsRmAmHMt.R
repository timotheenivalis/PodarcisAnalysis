...
setwd("C:/Users/Timothée//Documents/Studies/PodarcisCBGP/New simulations/sRmAmHMt/")

Simuls<-"sRmAmHMt0"
for (nbsimul in 1:17)
{
  Simuls<-c(Simuls,paste("sRmAmHMt",nbsimul,sep=""))
}

WrapMtFunc(Simuls = Simuls,FilePrefix = "sRmAmHMt",Param = rep(x = 1,times=length(Simuls)))
...
# 
# 
# write.table(results,file="sRmAmHMtresults.txt", sep="\t",quote=F,append=F,col.names=T)
# write.table(resultsnofix,file="sRmAmHMtresultsnofix.txt", sep="\t",quote=F,append=F,col.names=T)
# write.table(DistriMtMaxDF,file="sRmAmHMtDistriMtMax.txt", sep="\t",quote=F,append=F,col.names=T,row.names=F)
# 
# 
# for (nbsimul in 1:17)
# {
#   Simul<-paste("sRmAmHMt",nbsimul,sep="")
#   outR<-mtfunc(Simul)
#   results<-outR$results
#   resultsnofix<-outR$resultsNoFix
#   DistriMtMaxDF<-data.frame(rep(Simul,length(outR$DistriMtMax)),outR$DistriMtMax)
...
#   DistriMtMaxDF$Param<-1
#   
#   write.table(results,file="sRmAmHMtresults.txt", sep="\t",quote=F,append=T,col.names=F)
#   write.table(resultsnofix,file="sRmAmHMtresultsnofix.txt", sep="\t",quote=F,append=T,col.names=F)
#   write.table(DistriMtMaxDF,file="sRmAmHMtDistriMtMax.txt", sep="\t",quote=F,append=T,col.names=F,row.names=F)
# }
# 
# ##############################################################################################
# #############################################################################################
...
# simul<-read.table(file="sRmAmHMtresults.txt",header=T)
