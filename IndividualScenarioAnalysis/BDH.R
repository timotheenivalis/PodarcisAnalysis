...
simOrd<-c("BD1H8","BD1H6","BD1H7","BD3H8","BD3H6","BD3H7","BD6H8","BD6H6","BD6H7")

source(file="C:/Users/Timothée/Documents/Studies/PodarcisCBGP/New simulations/FunctionsSimul.R")
setwd("C:/Users/Timothée/Documents/Studies/PodarcisCBGP/New simulations/BDispHomog/")
...
write.table(results,file="BDHresults.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(resultsnofix,file="BDHresultsnofix.txt", sep="\t",quote=F,append=F,col.names=T)
write.table(DistriMtMaxDF,file="BDHDistriMtMax.txt", sep="\t",quote=F,append=F,col.names=T,row.names=F)
...
for (nbsimul in 2:9)
{
  ...
  write.table(results,file="BDHresults.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(resultsnofix,file="BDHresultsnofix.txt", sep="\t",quote=F,append=T,col.names=F)
  write.table(DistriMtMaxDF,file="BDHDistriMtMax.txt", sep="\t",quote=F,append=T,col.names=F,row.names=F)
}
...
##############################################################################################
#############################################################################################
simul<-read.table(file="BDHresults.txt",header=T)
