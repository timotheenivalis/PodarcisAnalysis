...
setwd("/home/timothee/Documents/Studies/PodarcisCBGP/New simulations/")
source(file="/home/timothee/Documents/Studies/PodarcisCBGP/New simulations/FunctionsSimul.R")
...
setwd("/home/timothee/Documents/Studies/PodarcisCBGP/Renew simulations")
...
plotDisc<-function(x,y,dmt,xlabb,transfo=identity,DrawLegend=FALSE,limY=limYP,limX=c(0,1), subind="", leftside=TRUE,...){
...
par(mar=c(5,3,2,3)+0.1)
  ...
  boxplot(disc~Param,data=dmt,axes=T,xaxt="n",yaxt="n",at=sort(transfo(x[,y])),boxwex=calcboxwex,border=colMt,boxlwd=lwdP,whisklwd=lwdP,staplelwd=lwdP,outlwd=lwdP,medlwd=lwdP,xlim=limX,xlab=xlabb,ylab="",cex.lab=cexP,las=1,cex.axis=cexP,ylim=limY,main="")
...
if(leftside==TRUE)
{
mtext("Mito-nuclear discordance",side=2,line=4,cex=cexP, las=0)
...
<<DiscordanceAll, dev='tikz', echo=F,out.width='\\textwidth',fig.align='center'>>=
...
# headers
#mtext("\\textbf{High introgression}", side=3, line=-2, outer=TRUE, cex=2, at =0)
mtext("\\textbf{Low introgression}", side=3, line=-1, outer=TRUE, cex=1, at = c(0.7,10))
